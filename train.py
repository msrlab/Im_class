...
def select_model(select_model):
    ...
    if select_model == 'densenet121_a':
        fl_model = setup_model('densenet121', [1024, 512, 256, 256], [0.2, 0.2, 0.2, 0.2], nr_out_features, nn.LogSoftmax(dim=1))
        criterion = nn.NLLLoss()
        optimizer = optim.Adam(fl_model.classifier.parameters(), lr=0.0001)

    if select_model == 'densenet121_b':
        fl_model = setup_model('densenet121', [512, 256], [0.5, 0.5], nr_out_features, nn.LogSoftmax(dim=1))
        criterion = nn.NLLLoss()
        optimizer = optim.Adam(fl_model.classifier.parameters(), lr=0.0001)   

    if select_model == 'vgg19_a':
        fl_model = setup_model('vgg19', [1600, 1600], [0.5, 0.5], nr_out_features, nn.LogSoftmax(dim=1))
        criterion = nn.NLLLoss()
        optimizer = optim.Adam(fl_model.classifier.parameters(), lr=0.0001) 

    if select_model == 'vgg16_a':
        fl_model = setup_model('vgg16', [1600, 1600], [0.5, 0.5], nr_out_features, nn.LogSoftmax(dim=1))
        criterion = nn.NLLLoss()
...
with active_session():
    log = train(fl_model, optimizer, criterion, epochs, eval_every_x_batch, eval_every_x_epoch, save_every_x_evalepoch, resume)
...
pypl.plot(train_log.val_loss, label='Validation Loss')
pypl.plot(train_log.train_loss, label='Train Loss')
pypl.legend()